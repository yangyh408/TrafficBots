...
class WOSACPostProcessing(nn.Module):
    ...
    def forward(self, batch: Dict[str, Tensor], buffer: RolloutBuffer) -> Dict[str, Tensor]:
        ...
        trajs = buffer.preds[:, :, :, buffer.step_future_start :]
        ...
        pos_no_sim = batch["history/agent_no_sim/pos"][..., :2].permute(0, 2, 1, 3)  # [n_sc, n_ag_no_sim, n_step_history, 3]
        yaw_no_sim = batch["history/agent_no_sim/yaw_bbox"].permute(0, 2, 1, 3)  # [n_sc, n_ag_no_sim, n_step_history, 1]
        ...
        wosac_data = {
            ...
            "valid_sim": batch["history/agent/valid"].permute(0, 2, 1),
            "pos_sim": pos_sim,  # [n_sc, n_joint_future, n_ag, n_step_future, 2]
            # "z_sim": batch["history/agent/pos"][..., 2:3],
            "z_sim": torch.zeros_like(batch["history/agent/pos"][..., 0: 1]).permute(0, 2, 1, 3),
            "yaw_sim": yaw_sim,  # [n_sc, n_joint_future, n_ag, n_step_future, 1]
            "valid_no_sim": batch["history/agent_no_sim/valid"].permute(0, 2, 1),
            ...
            "z_no_sim": torch.zeros_like(batch["history/agent_no_sim/pos"][..., 0: 1]).permute(0, 2, 1, 3),
