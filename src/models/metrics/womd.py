...
class WOMDMetrics(Metric):
    ...
    def __init__(self, prefix: str, step_gt: int, step_current: int, interactive_challenge: bool) -> None:
        """
    ...
    def update(self, batch: Dict[str, Tensor], pred_traj: Tensor, pred_score: Optional[Tensor] = None) -> None:
        ...
        ground_truth_trajectory = torch.zeros([n_batch, n_agent, n_gt_step, 7], dtype=torch.float32, device=device)
        ground_truth_is_valid = torch.zeros([n_batch, n_agent, n_gt_step], dtype=torch.bool, device=device)
        prediction_ground_truth_indices_mask = torch.zeros(
            [n_batch, self.m_joint, self.n_pred], dtype=torch.bool, device=device
        )
        object_type = torch.zeros([n_batch, n_agent], dtype=torch.float32, device=device)
